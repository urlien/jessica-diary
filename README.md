...
| 07-18 | 大暑 | 克莱因蓝主题 + 模型切换器 + 搜索优化 + 日记侧边栏 + 背景重构 |
| 09-16 | v3.5 白露 | 固定签名（覆盖安装）+ CI 调通 + 数据抢救 + 10 项修复（初始化 / CDN 404 / max_tokens / resumePendingReply / 主动消息清理 / 备份压缩 / 分片上传 / 导出 / 遗留收尾） |
| 09-17 | v3.6 秋分 | 天气重做(Open-Meteo) + 删知识库(RAG/V2KB) + 修背景/时间戳/卡顿 + 每日天气注入 |
